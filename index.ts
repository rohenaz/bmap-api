...
const start = async () => {
	...
	app.get(
		...
		asyncHandler(async (req, res) => {
			...
			const b64 = req.params.base64Query;
	...
	app.get(
		...
		asyncHandler(async (req, res) => {
			...
			try {
				...
				const { value } = await readFromRedis(countsKey);
				...
				for (const collection of bitcoinSchemaCollections) {
					...
					const { value } = await readFromRedis(timeSeriesKey);
  ...
  <div class="grid grid-cols-4 gap-8">
	...
	app.get(
		...
		asyncHandler(async (req, res) => {
			...
			const { value } = await readFromRedis(chartKey);
	...
	app.get("/query/:collectionName", (req, res) => {
		const collectionName = req.params.collectionName;
